...
parser.add_argument('context_size', type=int,
                    help='Number of tokens to consider in context.')
...
parser.add_argument('clip_norm', type=float, nargs='?', default=20.0,
                    help='Maximum L2 norm of mu. Defaults to 20.')
parser.add_argument('minimum', type=float, nargs='?', default=1e-3,
                    help='Minimum covariance eigenvalue. Defaults to 1e-3.')
parser.add_argument('maximum', type=float, nargs='?', default=1e3,
...
with open(args.data_file, 'r') as data_file:
    for line in tqdm(data_file.readlines()):
        ...
        mu = tf.clip_by_norm(mu, args.clip_norm)
        sigma = tf.maximum(args.minimum, tf.minimum(args.maximum, sigma))
